            ...
            for i, dq in enumerate(feature_dq_data):
                ...
                if checked:
                    ...
                    if dq["Measure"] in feature_decimal_measures:
                        ...
                        # Corresponding number measure
                        number_measure = rate_measures[dq["Measure"]]
                        # The index of the number measure
                        ...
                        if number_measure_index is not None:
                            # The total number of items from session state
                            total_items = st.session_state.get(f"{tab_prefix}_total_features", 0)
                            if total_items > 0:
                                # The number value from session state
            ...
            for i, dq in enumerate(feature_dq_data):
                if st.session_state.get(f"g0_feature_check_{i}", False):
                    ...
                    if dq["Measure"] in feature_decimal_measures:
                        ...
                        # The corresponding number measure
                        number_measure = rate_measures[dq["Measure"]]
                        # The index of the number measure
                        number_measure_index = next((j for j, d in enumerate(feature_dq_data) if d["Measure"] == number_measure), None)
...
with tab3:
    def create_verification_results():
        ...
        if mean_deviation != 0 and std_deviation != 0:
            ...
            st.write(f"**Deviation threshold δpos (filters out noise and identifies significant deviations):** {deviation_threshold:.3f}")
        else:
            st.write("**Deviation threshold δpos (filters out noise and identifies significant deviations):** Not calculated")
        ...
        if all(v is not None and v != 0 for v in [g0_abs_pos, g0_rel_pos, gt_abs_pos, gt_rel_pos]):
            try:
                ...
                st.write(f"**Accuracy difference between models Dacc (Controls and assesses the consistency of model accuracy over time):** {dacc:.3f}")
            except (ValueError, TypeError, ZeroDivisionError) as e:
                st.write("**Accuracy difference between models Dacc (Controls and assesses the consistency of model accuracy over time):** Not calculated")
        ...
        if dacc is not None and delta_d != 0:
            ...
            st.write(f"**Dacc vs. δD comparison (Indicates whether the model accuracy has remained consistent or declined over time):** {comparison_result}")
        else:
            st.write("**Dacc vs. δD comparison (Indicates whether the model accuracy has remained consistent or declined over time):** Not calculated")
        ...
        st.caption("Evaluates the consistency of LoD between Model G(0) and Model G(t)")
        ...
        if g0_lod != "N/D" and gt_lod != "N/D":
            ...
            st.write(f"**LoD Verification (Indicates whether the LoD remains consistent between models):** {lod_comparison}")
        else:
            st.write("**LoD Verification (Indicates whether the LoD remains consistent between models):** Not calculated")
        ...
        st.caption("Evaluates the consistency of resolution between Model G(0) and Model G(t)")
        ...
        if st.session_state.get('g0_model_resolution') is not None and st.session_state.get('gt_model_resolution') is not None:
            ...
            st.write(f"**Resolution Verification (Indicates whether the model resolution has improved or declined over time):** {resolution_comparison}")
        else:
            st.write("**Resolution Verification (Indicates whether the model resolution has improved or declined over time):** Not calculated")
        ...
        with col1:
            if st.button("📥 Download Results (CSV)", use_container_width=True):
                ...
                results = {
                    ...
                    "DQ Elements Summary": measures_summary_data,
                ...
                # Measures Summary
                writer.writerow(["DQ Elements Summary"])
