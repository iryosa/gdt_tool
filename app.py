...
with tab3:
    def create_verification_results():
        ...
        st.caption("Comparison of positional accuracy between Model G(0) and Model G(t). This condition evaluates whether Model G(t) maintains or improves the positional accuracy compared to Model G(0) by comparing the calculated Dacc value against the defined threshold δD.")
        ...
        st.caption("Consistency of LoD between Model G(0) and Model G(t). This condition ensures that both models maintain the same Level of Detail (LoD) requirements, indicating consistent geometric representation quality across model versions.")
        ...
        st.caption("Consistency of resolution between Model G(0) and Model G(t). This condition verifies that Model G(t) maintains or improves the spatial resolution compared to Model G(0), ensuring no degradation in model quality during the updating process.")
        ...
        st.markdown('<h3 style="color: #89A8B2; font-size: 1.2rem; font-style: italic;">Performance Comparison: Model G(t) vs Model G(0) (optional)</h3>', unsafe_allow_html=True)
